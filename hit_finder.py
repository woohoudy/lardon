...
def recompute_hit_charge(hit):
    ...
    val = 0.
    shape_pos_val, shape_neg_val = 0., 0.
    mean = dc.evt_list[-1].noise_filt.ped_mean[daq_ch]

    if(sig == "Collection"):
        for t in range(start, stop):
            val += dc.data_daq[daq_ch, t] - mean
            shape_pos_val += abs(dc.data_daq[daq_ch, t])

        hit.charge_pos = val
        hit.charge_neg = -0.
        hit.shape_abs_pos = shape_pos_val
        hit.shape_abs_neg = shape_neg_val
        ...
        for t in range(zero, stop):
            val += dc.data_daq[daq_ch, t] + mean
            shape_neg_val += abs(dc.data_daq[daq_ch, t])
        hit.charge_neg = val
        hit.shape_abs_pos = shape_pos_val
        hit.shape_abs_neg = shape_neg_val
